...
// aloca-se mem�ria para o descritor (explicado no �nicio deste arquivo) e faz o
// seus ponteiros apontarem para o Nulo
...
void imprime_listad(tp_listad *lista, int ordem) {
  if (lista == NULL)
    ...
    case 1:
      atu = lista->ini;
      while (atu != NULL) {
        printf("[%d|%d] ", atu->info.lado_esq, atu->info.lado_dir);
        atu = atu->prox;
      }
      break;
    case 2:
...
int remove_listad(tp_listad *lista, tp_item e) {
  tp_no *atu;
  atu = lista->ini;
  while ((atu != NULL) && (atu->info.lado_esq != e.lado_esq || atu->info.lado_dir != e.lado_dir)) {
    atu = atu->prox;
  }
  if (atu == NULL)
    return 0;
